...
(defun %scaled-product-vector-array (scalar vector array &optional result)
...
(defun %array<-array1-op-array2 (operation array1 array2)
  (let ((m-rows (array-dimension array1 0))
        ...
        (result (make-array (array-dimensions array1) :element-type (common-array-element-type array1 array2))))
    (dotimes (row m-rows result)
      (dotimes (column n-columns)
        (setf (aref result row column) (funcall operation (aref array1 row column) (aref array2 row column)))))))
...
(defmethod compatible-dimensions-p
  ...
  (and (= 2 (array-rank array1) (array-rank array2))
