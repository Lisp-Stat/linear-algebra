...
(defun %vector<-vector1-op-vector2 (operation vector1 vector2)
  ...
  (let ((result (make-array (length vector1) :element-type (common-array-element-type vector1 vector2))))
    (dotimes (index (length vector1) result)
      (setf (aref result index) (funcall operation (aref vector1 index) (aref vector2 index))))))
...
(defun %vector1<-vector1-op-vector2 (operation vector1 vector2)
  ...
  (dotimes (index (length vector1) vector1)
    (setf (aref vector1 index) (funcall operation (aref vector1 index) (aref vector2 index)))))
...
(defun %product-vector-array (vector array &optional result)
  ...
  (loop
    with m-rows = (array-dimension array 0) and n-columns = (array-dimension array 1)
...
(defun %product-array-array (array1 array2 &optional result)
  ...
  (loop
    with m-rows = (array-dimension array1 0) and l-columns = (array-dimension array1 1)
