...
(defun symmetric-cholesky-decomposition (array)
  ...
  (loop
    ...
    for index-j below size do
      ;; Step 1.1
      (multiple-value-bind (scale sumsq) (sumsq-row array index-j :end index-j)
	(setf (aref array index-j index-j)
	      (sqrt (- (aref array index-j index-j) (* scale scale sumsq)))))
      ;; Step 1.2
      (loop
	for index-k from (1+ index-j) below size
	as element = (/ (- (aref array index-k index-j)
			   (loop
			     for index-i below index-j
			     sum (* (aref array index-k index-i)
				    (aref array index-j index-i))))
			(aref array index-j index-j))
	do (setf (aref array index-k index-j) element
    ...
    do (loop
	 ...
	 do (setf
	    ...
	    (loop
	      ...
	      do ;; Lower
		 (setf (aref array index-j index-k) element
...
(defun symmetric-cholesky-solver (array vector)
  ...
  (let ((size (array-dimension array 0)))
    ...
    (loop
      ...
      do (loop
	   for index-i below index-j
	   do (decf (aref vector index-j)
		    (* (aref array index-j index-i) (aref vector index-i)))))
    ;; Step 2.3 & 3.2
    (loop
      for index-j from (1- size) downto 0
      do (setf (aref vector index-j)
	       (/ (aref vector index-j) (aref array index-j index-j)))
	 (loop
	   for index-i from (1+ index-j) below size
	   do (decf (aref vector index-j)
...
(defun symmetric-cholesky-invert (array)
  ...
  (loop
    ...
    with array^-1 = (make-array (array-dimensions array) :element-type element-type :initial-element (coerce 0 element-type))
      initially
	 ;; Step 1, decomposition
	 (root-free-symmetric-cholesky-decomposition array)
    for index-k below size
    do (setf (aref array^-1 index-k index-k) one)
       ;; Step 2.1 & 2.2
       (loop
	 for index-j from (1+ index-k) below size
	 do (loop
	      for index-i from index-k below index-j
	      do (decf (aref array^-1 index-j index-k)
		       (* (aref array index-j index-i)
			  (aref array^-1 index-i index-k)))))
       ;; Step 2.3 & 3.2
       (loop
	 for index-j from (1- size) downto index-k
	 do (setf (aref array^-1 index-j index-k)
	    ...
	    (loop
	      ...
	      do (decf (aref array^-1 index-j index-k)
			  ...
			  (aref array^-1 index-i index-k))))
	    (setf (aref array^-1 index-k index-j)
		  (aref array^-1 index-j index-k)))
