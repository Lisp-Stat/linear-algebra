...
(defgeneric right-permute (vector-or-array permutation)
  (:documentation "Permute the row vector or columns of the array."))

(defmethod right-permute ((data vector) (permutation vector))
  "Permute the row vector to create a column vector."
  (loop
    with result = (make-array (length data) :element-type (array-element-type data))
...
(defmethod right-permute ((data array) (permutation vector))
  ...
  (loop
    ...
    with result = (make-array (array-dimensions data) :element-type (array-element-type data))
    for column across permutation
    and row = 0 then (1+ row)
    do (loop
	 for irow below m-rows
	 do (setf (aref result irow column) (aref data irow row)))
    finally (return result)))

;;; Left permutation

(defgeneric left-permute (permutation vector-or-array)
  (:documentation "Permute the column vector or rows of the array."))
...
(defmethod left-permute ((permutation vector) (data array))
  ...
  (loop
    ...
    and row = 0 then (1+ row)
    do (loop
	 for icol below n-columns
	 do (setf (aref result row icol) (aref data column icol)))
    finally (return result)))
