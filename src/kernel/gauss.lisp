...
(defun column-pivot-search (array column)
  ...
  (loop
    ...
    when (< max-element element)
      do (setq max-row row max-element element)
    finally (return max-row)))

(defun swap-rows (array i0 jth)
  "Interchange the "
  (loop
    for column below (array-dimension array 1)
    do (rotatef (aref array i0 column) (aref array jth column))
...
(defun column-pivot (array pivot-selection-vector column)
  ...
  (let ((i0 (column-pivot-search array column)))
    (unless (= i0 column)
      (rotatef (svref pivot-selection-vector i0)
	       (svref pivot-selection-vector column))
      (swap-rows array i0 column)))
  ;; Check for a singular matrix
  (when (num= 0.0 (aref array column column))
  ...
  (loop
    ...
    ;; Step 2.2.1
    do (setf (aref array row column)
	     (/ (aref array row column) pivot-column-element))

       ;; Step 2.2.2
       (loop
	 for col from (1+ column) below size
...
(defun gauss-update (factored pivot-selection-vector vector)
  ...
  (loop
    with update = (make-array (length vector) :element-type (array-element-type vector)
    ...
    do (setf (aref update row) (- (aref vector(svref pivot-selection-vector row))
				  (loop
...
(defun gauss-solver (array vector)
  "Gauss algorithm with column pivot search."
  (multiple-value-bind (factored pivot-selection-vector) (gauss-factorization array)
    (gauss-backsubstitution factored (gauss-update factored pivot-selection-vector vector))))

;;; Algorithm 4.25, pg. 77
;;; Finding A^-1 via Gauss algorithm with partial column pivot search

(defun unit-pivot-value (pivot-selection-vector row column
  ...
  (coerce (if (= column (svref pivot-selection-vector row)) 1.0 0.0)
	  array-type))
...
(defun gauss-invert (array)
  ...
  (let* ((size (array-dimension array 0))
         ...
         (solution (make-array (list size size) :element-type array-type
						:initial-element (coerce 0.0 array-type))))
    ;; Step 1
    (multiple-value-bind (factored pivot-selection-vector) (gauss-factorization array)
      ;; Step 2
      (loop
	for column below size
	do (setf (aref solution 0 column) (unit-pivot-value pivot-selection-vector 0 column array-type))
	   (loop
	     for row from 1 below size
	     do (setf (aref solution row column)
		      (- (unit-pivot-value
			  pivot-selection-vector row column array-type)
			 (loop for index from 0 below row
			       sum (* (aref factored row index)
				      (aref solution index column)))))))
      ;; Step 3
      (loop
	with end = (1- size)
	for column below size
	do (setf (aref solution end column)
	   ...
	   (loop
		 ...
		 (/ (- (aref solution row column)
		       (loop for index from (1+ row) below size
			     sum (* (aref factored row index)
				    (aref solution index column))))
		    (aref factored row row))))))
    ;; Return the solution
