...
(defmethod initialize-matrix-contents :before ((matrix square-matrix) initial-contents initargs)
...
(defmethod submatrix ((matrix square-matrix) (start-row integer) (start-column integer) &key end-row end-column)
  ...
  (multiple-value-bind (start-row start-column end-row end-column)
    ...
    (let* ((m-rows (- end-row start-row))
      ...
      (make-instance (if (= m-rows n-columns) 'square-matrix 'dense-matrix)
...
(defmethod compatible-dimensions-p ((operation (eql :solve)) (matrix square-matrix) (vector column-vector))
...
(defmethod invert ((matrix square-matrix))
  ...
  (make-instance (class-of matrix) :contents (gauss-invert (copy-array (contents matrix)))))

(defmethod ninvert ((matrix square-matrix))
  "Return the invert of the square matrix."
  (make-instance (class-of matrix) :contents (gauss-invert (contents matrix))))
